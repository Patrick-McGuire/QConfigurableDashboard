 ...
 */
#define QCD_TUNNEL_CALLBACK(ptr, func) std::bind(&std::remove_pointer<decltype(ptr)>::type::func, ptr, std::placeholders::_1, std::placeholders::_2)
...
namespace QCD {
    ...
    typedef std::function<void(Json, std::string)> TunnelCallback;                  ///< Callback format used underlying our implementation of events
    ...
    const char *const THEME_CHANGED_CALLBACK = "themeChanged";                      ///< Reserved event identifier for when the theme changes
    const char *const MENU_BASE_CALLBACK = "menuEvent";                             ///< Reserved prefix event identifier for menu events

    const char *const TICK_TIME_KEY = "tickTime";                                   ///< Reserved input data key updated with the loop time of the application
    const char *const TICK_RATE_KEY = "tickRate";                                   ///< Reserved input data key updated with the runUpdate rate of the application
    const char *const TICK_DESIRED_RATE_KEY = "tickDesRate";                        ///< Reserved input data key updated with the desired runUpdate rate of the application

    const char *const VIDEO_RECORD_CALLBACK = "recordVideo";                        ///< Reserved callback identifier for starting/stopping recording of video
     ...
     */
    enum WidgetFocus {
        NO_FOCUS,        ///< Widget is not currently visible (except on first runUpdate when coming out of focus, then value is ON_NO_FOCUS)
        IN_FOCUS,        ///< Widget is visible (except on first runUpdate when coming into focus, then value is ON_IN_FOCUS)
        ON_IN_FOCUS,     ///< First runUpdate when Widget is coming into focus
        ON_NO_FOCUS,     ///< First runUpdate when Widget is coming out of focus
    };
}
